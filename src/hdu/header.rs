...
fn check_card_keyword(card: &[u8; 80], keyword: &[u8]) -> Result<card::Value, Error> {
...
impl PrimaryHeader {
    pub(crate) fn parse<R: BufRead>(reader: &mut R, bytes_read: &mut usize) -> Result<Self, Error> {
        ...
        let mut naxis_size = (0..naxis)
            .map(|idx_axis| {
                consume_next_card(reader, &mut card_80_bytes_buf)?;
                let size = check_card_keyword(&card_80_bytes_buf, NAXIS_KW[idx_axis])?
                    .check_for_integer()?;
