...
where X: Xtension + Debug
{
  let data_starting_byte = hdu.get_data_unit_byte_offset();
  let data_length_byte = hdu.get_data_unit_byte_size();
  println!(" * DATA starting byte: {}; byte size: {}.", data_starting_byte, data_length_byte);
}

fn print_img_header(img: &Image) {
  println!(" * HEAD naxis: {}; bitpix : {:?}; dimensions: {}.",
           img.get_naxis(),
           img.get_bitpix(),
           img.get_naxisn_all()
             .iter()
...
fn print_bintable_header(bin: &BinTable) {
  println!(" * HEAD n_cols: {}; n_rows: {}.", bin.get_num_cols(), bin.get_num_rows());
}

fn print_asciitable_header(ascii: &AsciiTable) {
  println!(" * HEAD n_cols: {}; n_rows: {}.", ascii.get_num_cols(), ascii.get_num_rows());
}